...
全站只用三种状态色 + 一种品牌色；**不使用绿色**（历史遗留的 emerald/green 已全部替换为天蓝 sky）。

| 语义 | 文字 | 浅底 | 描边 / 实底 | 用途 |
| --- | --- | --- | --- | --- |
| 成功 / 正常 / 健康 | `sky-700`（深色 `sky-400`） | `sky-50` / `dark:sky-500/10` | `sky-200` / 实底 `sky-700`（白字） | 运行正常、验证通过、支付成功、健康指标 |
...
天蓝色阶（sky）：`50 #F0F9FF` / `100 #E0F2FE` / `200 #BAE6FD` / `300 #7DD3FC` / `400 #38BDF8` /
`500 #0EA5E9` / `600 #0284C7` / `700 #0369A1`。

> 数据可视化里的分类色板同样不含绿色：原本的 green/emerald/teal/lime 系列位已改为
> `sky` 与 `cyan` 的天蓝色调（保持同一张图表内颜色不重复）。
...
7. **不要使用绿色系**（`emerald` / `green` / `lime` / `teal` 及其 hex/rgb 写法）：
   状态「正常/成功/健康」统一用天蓝 `sky` 系；图表分类色板同样不含绿色。
