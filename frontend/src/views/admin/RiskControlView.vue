              ...
              >
                ...
                >
                  <div class="flex min-w-0 flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div class="grid grid-cols-4 gap-2 text-right text-xs text-gray-500 dark:text-gray-400 sm:min-w-[280px]">
                      <div>
                        ...
                        <p class="mt-1 text-sm font-semibold text-primary-700 dark:text-primary-300">{{ formatNumber(item.active) }}</p>
                      </div>
                      <div>
                  ...
                  <div class="mt-3 h-1.5 overflow-hidden rounded-full bg-white dark:bg-dark-900">
                    <div class="h-full rounded-full bg-primary-500" :style="{ width: preBlockAPIKeyLoadWidth(item.total) }"></div>
                  </div>
                </div>
              </div>
          ...
          <div class="grid grid-cols-1 gap-6 p-6 xl:grid-cols-[minmax(0,360px)_1fr]">
            <div class="space-y-4">
              ...
              <div class="grid grid-cols-2 gap-3">
                ...
                <div class="rounded-lg bg-primary-50 p-4 dark:bg-primary-900/10">
                  <p class="text-xs text-gray-500 dark:text-gray-400">{{ t('admin.riskControl.idleWorkers') }}</p>
                  <p class="mt-2 text-2xl font-semibold text-primary-700 dark:text-primary-400">{{ status?.idle_workers ?? configForm.worker_count }}</p>
                </div>
                <div class="rounded-lg bg-gray-50 p-4 dark:bg-dark-700/50">
                  ...
                  <tr v-for="row in logs" :key="row.id" class="hover:bg-gray-50 dark:hover:bg-dark-700/60">
                    ...
                    <td class="whitespace-nowrap px-5 py-4 text-sm text-gray-700 dark:text-gray-300">
                      ...
                      <button
                        ...
                        type="button"
                        class="mt-2 inline-flex items-center gap-1 rounded-md border border-primary-200 bg-primary-50 px-2 py-1 text-xs font-medium text-primary-700 transition-colors hover:bg-primary-100 disabled:cursor-not-allowed disabled:opacity-60 dark:border-primary-900/60 dark:bg-primary-900/20 dark:text-primary-400 dark:hover:bg-primary-900/30"
                ...
                <div data-test="audit-key-statuses" class="rounded-lg border border-gray-100 bg-gray-50 p-3 dark:border-dark-700 dark:bg-dark-900/30">
                          ...
                          <div class="min-w-0">
                            <div class="flex min-w-0 flex-wrap items-center gap-2">
                              ...
                              <span
                                ...
                                :class="row.configured ? 'bg-primary-50 text-primary-700 dark:bg-primary-900/30 dark:text-primary-300' : 'bg-primary-50 text-primary-700 dark:bg-primary-500/15 dark:text-primary-300'"
                              >
                  ...
                  <div v-if="moderationTestResult" class="mt-4 rounded-lg border border-gray-100 bg-white p-3 dark:border-dark-700 dark:bg-dark-800">
                    ...
                    <div class="flex items-start justify-between gap-3">
                      ...
                      <span class="inline-flex rounded-full px-2 py-1 text-xs font-medium" :class="moderationTestResult.flagged ? 'bg-red-50 text-red-700 dark:bg-red-900/20 dark:text-red-300' : 'bg-primary-50 text-primary-700 dark:bg-primary-900/20 dark:text-primary-400'">
                    ...
                    <div class="mt-3">
                      ...
                      <div class="h-2 overflow-hidden rounded-full bg-gray-100 dark:bg-dark-700">
                        <div class="h-full rounded-full" :class="moderationTestResult.flagged ? 'bg-red-500' : 'bg-primary-500'" :style="{ width: percentWidth(moderationTestResult.composite_score) }"></div>
                      </div>
                    </div>
        ...
        <div v-if="inputDetailRow" class="space-y-5">
          ...
          <div class="rounded-xl border border-gray-100 bg-white p-4 shadow-sm dark:border-dark-700 dark:bg-dark-800">
            <div class="flex flex-wrap items-center justify-between gap-3">
              ...
              <span v-if="inputDetailRow.group_name" class="inline-flex rounded-md bg-primary-50 px-2.5 py-1 text-xs font-medium text-primary-700 dark:bg-primary-500/15 dark:text-primary-300">
...
const overviewItems = computed<OverviewItem[]>(() => [
  ...
  },
  {
    ...
    icon: 'key',
    iconClass: 'bg-primary-50 text-primary-600 dark:bg-primary-500/15 dark:text-primary-300',
  },
  {
    key: 'scope',
...
const preBlockMetricItems = computed(() => [
  ...
  },
  {
    ...
    valueClass: 'text-primary-700 dark:text-primary-400',
  },
  {
    key: 'blocked',
...
function workerSlotClass(state: WorkerSlotState): string {
  if (state === 'active') {
    return 'border-primary-200 bg-primary-50 text-primary-700 dark:border-primary-500/25 dark:bg-primary-500/15 dark:text-primary-300'
  }
  if (state === 'idle') {
    return 'border-primary-200 bg-primary-50 text-primary-700 dark:border-primary-900/60 dark:bg-primary-900/20 dark:text-primary-400'
  }
...
function workerDotClass(state: WorkerSlotState): string {
  if (state === 'active') return 'bg-primary-500'
  if (state === 'idle') return 'bg-primary-500'
...
function apiKeyStatusBadgeClass(statusValue: ContentModerationAPIKeyStatus['status']): string {
  const classes: Record<ContentModerationAPIKeyStatus['status'], string> = {
    ok: 'bg-primary-50 text-primary-700 dark:bg-primary-900/20 dark:text-primary-400',
