            ...
            <div class="space-y-4">
              ...
              <div class="grid grid-cols-2 gap-3">
                ...
                <div class="rounded-lg bg-sky-50 p-4 dark:bg-sky-900/10">
                  ...
                  <p class="mt-2 text-2xl font-semibold text-sky-700 dark:text-sky-400">{{ status?.idle_workers ?? configForm.worker_count }}</p>
                </div>
                <div class="rounded-lg bg-gray-50 p-4 dark:bg-dark-700/50">
                  ...
                  <tr v-for="row in logs" :key="row.id" class="hover:bg-gray-50 dark:hover:bg-dark-700/60">
                    ...
                    <td class="whitespace-nowrap px-5 py-4 text-sm text-gray-700 dark:text-gray-300">
                      ...
                      <button
                        ...
                        type="button"
                        class="mt-2 inline-flex items-center gap-1 rounded-md border border-sky-200 bg-sky-50 px-2 py-1 text-xs font-medium text-sky-700 transition-colors hover:bg-sky-100 disabled:cursor-not-allowed disabled:opacity-60 dark:border-sky-900/60 dark:bg-sky-900/20 dark:text-sky-400 dark:hover:bg-sky-900/30"
                ...
                <div class="rounded-lg border border-gray-100 bg-gray-50 p-3 dark:border-dark-700 dark:bg-dark-900/30">
                  ...
                  <div v-if="moderationTestResult" class="mt-4 rounded-lg border border-gray-100 bg-white p-3 dark:border-dark-700 dark:bg-dark-800">
                    <div class="flex items-start justify-between gap-3">
                      ...
                      <span class="inline-flex rounded-full px-2 py-1 text-xs font-medium" :class="moderationTestResult.flagged ? 'bg-red-50 text-red-700 dark:bg-red-900/20 dark:text-red-300' : 'bg-sky-50 text-sky-700 dark:bg-sky-900/20 dark:text-sky-400'">
                    ...
                    <div class="mt-3">
                      ...
                      <div class="h-2 overflow-hidden rounded-full bg-gray-100 dark:bg-dark-700">
                        <div class="h-full rounded-full" :class="moderationTestResult.flagged ? 'bg-red-500' : 'bg-sky-500'" :style="{ width: percentWidth(moderationTestResult.composite_score) }"></div>
                      </div>
                    </div>
...
function workerSlotClass(state: WorkerSlotState): string {
  ...
  if (state === 'idle') {
    return 'border-sky-200 bg-sky-50 text-sky-700 dark:border-sky-900/60 dark:bg-sky-900/20 dark:text-sky-400'
  }
...
function workerDotClass(state: WorkerSlotState): string {
  ...
  if (state === 'idle') return 'bg-sky-500'
...
function apiKeyStatusBadgeClass(statusValue: ContentModerationAPIKeyStatus['status']): string {
  const classes: Record<ContentModerationAPIKeyStatus['status'], string> = {
    ok: 'bg-sky-50 text-sky-700 dark:bg-sky-900/20 dark:text-sky-400',
