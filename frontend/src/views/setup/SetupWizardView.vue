<template>
  <div
    class="flex min-h-screen items-center justify-center bg-white p-4 dark:bg-dark-950"
  >
    <div class="w-full max-w-2xl">
      <!-- Logo & Title -->
      <div class="mb-8 text-center">
        <div
          class="mb-4 inline-flex h-16 w-16 items-center justify-center rounded-2xl border border-primary-100 bg-primary-50 text-primary-600 dark:border-primary-500/20 dark:bg-primary-500/10 dark:text-primary-300"
        >
          <Icon name="cog" size="xl" />
        </div>
        <h1 class="text-3xl font-semibold tracking-tight text-gray-900 dark:text-white">{{ t('setup.title') }}</h1>
              ...
              <div
                :class="[
                  ...
                  currentStep > index
                    ...
                    : currentStep === index
                      ? 'bg-primary-500 text-white ring-2 ring-primary-100 dark:ring-primary-500/25'
      ...
      <div class="card p-8">
        ...
        <div v-if="currentStep === 0" class="space-y-6">
          ...
          >
            ...
            <Icon v-else-if="dbConnected" name="check" size="md" class="mr-2 text-sky-500" :stroke-width="2" />
            {{
              testingDb
        ...
        <div
          v-if="errorMessage"
          class="mt-6 rounded-xl border border-red-200 bg-red-50 p-4 dark:border-red-500/25 dark:bg-red-500/10"
        >
        ...
        <div
          v-if="installSuccess"
          class="mt-6 rounded-xl border border-sky-200 bg-sky-50 p-4 dark:border-sky-400/25 dark:bg-sky-400/10"
        >
          <div class="flex items-start gap-3">
            <svg
              v-if="!serviceReady"
              class="h-5 w-5 flex-shrink-0 animate-spin text-sky-500"
              fill="none"
              viewBox="0 0 24 24"
            >
              <circle
            ...
            <Icon v-else name="checkCircle" size="md" class="flex-shrink-0 text-sky-500" />
            <div>
              <p class="text-sm font-medium text-sky-700 dark:text-sky-400">
                {{ t('setup.status.completed') }}
              </p>
              <p class="mt-1 text-sm text-sky-700 dark:text-sky-400">
                {{
                  serviceReady
