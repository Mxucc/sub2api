      ...
      <div class="rounded-xl bg-gray-50 p-4 dark:bg-dark-700">
        ...
        <div class="mt-2.5 flex items-center justify-between border-t border-gray-200/60 pt-2.5 dark:border-dark-600/60">
          ...
          <p class="ml-4 flex-shrink-0 text-xs text-gray-500 dark:text-dark-400">
            {{ t('admin.users.totalRecharged') }}: <span class="font-semibold text-primary-700 dark:text-primary-400">${{ totalRecharged.toFixed(2) }}</span>
          </p>
        </div>
      </div>
...
const getIconBg = (item: BalanceHistoryItem) => {
  ...
  if (isSubscriptionType(item.type)) return 'bg-primary-100 dark:bg-primary-500/15'
  return item.value >= 0
...
const getIconColor = (item: BalanceHistoryItem) => {
  ...
  if (isSubscriptionType(item.type)) return 'text-primary-600 dark:text-primary-300'
  return item.value >= 0
