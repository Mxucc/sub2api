  ...
  <div>
    ...
    >
      <span class="collapse-head-title">
        <Icon name="clock" size="sm" class="shrink-0 text-gray-400 dark:text-dark-500" />
      ...
      <span class="flex items-center gap-2">
        ...
        <Icon
          name="chevronDown"
          size="sm"
          :class="['shrink-0 text-gray-400 transition-transform duration-200 dark:text-dark-500', !expanded && '-rotate-90']"
        />
      </span>
    </button>
        ...
        <div class="ledger mt-2">
          ...
          <div v-for="log in data" :key="log.id" class="ledger-row ledger-row-hover">
            <div class="flex min-w-0 items-start gap-2">
              <Icon name="sparkles" size="sm" class="mt-0.5 shrink-0 text-gray-400 dark:text-dark-500" />
              <div class="min-w-0">
            ...
            <span class="ledger-num text-gray-600 dark:text-dark-400">
              ...
              <span class="ml-1 text-2xs text-gray-400 dark:text-dark-500">{{ t('dashboard.tokens') }}</span>
            </span>
            <span class="ledger-num-strong flex flex-col items-end gap-0.5">
              <span class="metric-accent" :title="t('dashboard.actual')">${{ formatCost(log.actual_cost) }}</span>
              <span class="text-2xs font-normal text-gray-400 dark:text-gray-500" :title="t('dashboard.standard')">${{ formatCost(log.total_cost) }}</span>
            </span>
          </div>
        </div>
