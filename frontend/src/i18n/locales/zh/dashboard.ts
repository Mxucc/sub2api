export default {
  ...
  modelPlaza: {
    ...
    table: {
      ...
      perRequest: '按次计费',
      perImage: '按图片计费',
      // 计费表达式分档价（billing_expr）：官方价格列只是基线，真实单价按时段/档位列出
      timeTierTitleTime: '时段价',
      timeTierTitleTiers: '分档价',
      timeTierCaption: '{model} 的分档单价',
      timeTierUnitPerMillion: '$ / 1M',
      timeTierExpand: '展开',
      timeTierCollapse: '收起',
      timeTierTierColumn: '时段 / 档位',
      timeTierConditionColumn: '生效条件',
      timeTierFlatColumn: '每次请求',
      timeTierFlatUnit: '$ / 次',
      timeTierUnconditional: '其余情况',
      timeTierUnnamedTier: '档位 {index}',
      timeTierNamePeak: '高峰',
      timeTierNameOffPeak: '低谷',
      timeTierNameBase: '标准',
      timeTierSourceCatalog: '价格表',
      timeTierSourceBuiltin: '内置',
      timeTierBaselineBadge: '基线',
      timeTierBaselineNote: '官方价格列只作基线参考，未含时段差异；各模型的实际单价以其分档表为准',
      timeTierUnitNote: '分档单价为官方口径（每 1M token），不含分组倍率',
      timeTierRawNote: '该模型按计费表达式计价，不是可识别的时间分档形态，以下为原始表达式',
      varInput: '输入',
      varOutput: '输出',
      varCacheRead: '缓存命中',
      varCacheWrite: '缓存写入',
      varCacheWrite1h: '缓存写入（1h）',
      varImageInput: '图片输入',
      varImageCacheRead: '图片缓存命中',
      varAudioInput: '音频输入',
      varAudioOutput: '音频输出'
    },
    nav: {
